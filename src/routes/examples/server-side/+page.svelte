<script lang="ts">
	...
	const tableState = new ServerTableState<Person>({
		columns: [
			{
	...
	{#if tableState.error}
		...
		>
			<div class="flex items-center gap-2 text-sm text-red-800">
				...
				<span>{tableState.error.message}</span>
			</div>
			<button
				onclick={() => {
	...
	{#if anyFiltered}
		<div class="mb-3 flex items-center gap-3">
			<span class="text-sm text-gray-600">
				Showing <strong>{tableState.rowCount}</strong> matching rows
			</span>
			<button
	...
	<div class="mb-3 flex items-center gap-2">
		...
		{#if tableState.searchQuery}
			<span class="text-sm text-gray-500">
				{tableState.rowCount} result{tableState.rowCount === 1 ? '' : 's'}
			</span>
		{/if}
	</div>

	<!-- Table wrapper — relative so the loading overlay can be positioned over it -->
	<div class="relative">
		<!-- Loading Overlay -->
		...
		<div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
			<table class="w-full border-collapse text-sm">
				...
				<tbody>
					{#each tableState.rows as row, i (row)}
						<tr
							class={[
	...
	<div class="mt-4 flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex items-center gap-2">
			...
			<span class="px-2 text-sm text-gray-600">
				...
				&nbsp;|&nbsp;
				<strong>{tableState.rowCount}</strong> results
			</span>
